...
def eval(feature_name, feature_file, sub, save_regressed_y = False):  
    # results_save_dir = "../../Research/pred/meg_sub"+str(sub)+"_predictions/"
    ...
    if sub<10:
        data = np.load('../subjects/sub0'+str(sub)+'-meg-data-ses0.npy', allow_pickle=True)
    else:
    ...
    for eachlayer in np.arange(1):   
        ...
        for train, test in kf.split(np.arange(4)):
            #for BERT
            x_train = np.concatenate([np.array(features[train[0]][eachlayer]), np.array(features[train[1]][eachlayer]), np.array(features[train[2]][eachlayer])], axis=0)
            ...
            # np.save(os.path.join(save_dir, "{}_lbda_{}".format(str(eachlayer), split_num)),lbda)
            
            # if save_regressed_y:
            #     y_pred_train = np.dot(x_train,weights)
            #     np.save(os.path.join(save_dir, "{}_y_regress_test_{}".format(str(eachlayer), split_num)),y_test - y_pred)
            #     np.save(os.path.join(save_dir, "{}_y_regress_train_{}".format(str(eachlayer), split_num)),y_train - y_pred_train)
...
allfeatures = ['pengi_text_joint_embeddings.npy']
